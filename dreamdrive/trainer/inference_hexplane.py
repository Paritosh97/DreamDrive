...
def render_sets(
    ...
    if args.get_video:
        image_folder = os.path.join(dataset.model_path, f'{render_key}/ours_{args.iteration}/renders')
...
def render_sets_hexplane(
    ...
    with torch.no_grad():
        ...
        scene = HexPlaneScene(dataset, gaussians, load_iteration=iteration, opt=args, shuffle=False, render=render_key)
...
def inference(
    ...
    with torch.no_grad():
        ...
        dynamic_mask = cluster_mask_v2(dynamic_scores, cluster_ids, th=cluster_threshold) # [N, 1]

...
def training_dreamdrive(dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    progress_bar = tqdm(range(opt.dynamic_iterations), desc="Dynamic Model Training Progress")
    for iteration in range(1, opt.dynamic_iterations + 1):
        ...
        dynamic_scores = dynamic.step(gaussians.get_semantic_features)
        dynamic_mask = dynamic_scores #differential
        ...
        dynamic_pred = feature_map[:1] # [1, H, W] for numerical stability
        # dynamic_pred = torch.clamp(dynamic_pred, min=0, max=1)

        # Loss
        gt_image = viewpoint_cam.original_image.cuda() # [3, H, W]
        gt_featmap = viewpoint_cam.feature_map.permute(2, 0, 1) # [H, W, C] -> [C, H, W]
        ...
        with torch.no_grad():
            ...
            # rendered_featmap = feature_map[1:] # [C, H, W]
            # norm_map = torch.abs(gt_featmap - rendered_featmap).mean(dim=0).unsqueeze(0) # [1, H, W]
            # norm_map = (norm_map - norm_map.min()) / (norm_map.max() - norm_map.min())
            gt_dynamic_mask = norm_map.detach() # as target to train dynamic model
        # save_heatmap(gt_dynamic_mask.squeeze(0), scene.model_path + "/dynamic/" + f"{viewpoint_cam.image_name}.png")
        # save_image(image.detach(), scene.model_path + "/dynamic/" + f"{viewpoint_cam.image_name}_render.png")
        # for th in [0.1, 0.3, 0.5, 0.7, 0.9]:
        #     gt_dynamic_mask_th = (gt_dynamic_mask > th).float() # (img_norm > opt.dynamic_th).float() # [1, H, W]
        #     save_heatmap(gt_dynamic_mask_th.squeeze(0), scene.model_path + "/dynamic/" + f"{viewpoint_cam.image_name}_mask_{th}.png")
        # import pdb; pdb.set_trace()
        # opt.dynamic_th = 0.3
        gt_dynamic_mask = (gt_dynamic_mask > dataset.dynamic_th).float() # (img_norm > opt.dynamic_th).float() # [1, H, W]

        loss = sigmoid_bce_loss(dynamic_pred, gt_dynamic_mask) # gt_dynamic_mask is a soft mask [0, 1]
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{7}f}"})
                progress_bar.update(10)
            if iteration == opt.dynamic_iterations:
                progress_bar.close()

            # Optimizer step
            if iteration < opt.dynamic_iterations:
                dynamic.optimizer.step()
                dynamic.optimizer.zero_grad()
                dynamic.update_learning_rate(iteration)    
    ...
    dynamic_scores = dynamic.step(gaussians.get_semantic_features)
    cluster_threshold = 0.75 # dataset.cluster_th
    dynamic_mask = cluster_mask_v2(dynamic_scores, cluster_ids, th=cluster_threshold) # [N, 1] 38087 standard
    dynamic_mask = dynamic_mask.detach() # non-differentiablse
    print(f"number of dynamic points: {dynamic_mask.sum().item()}")
    
    # obtain fake dynamic gaussians
    all_dynamic_points = (dynamic_scores >= 0.15).float() # hacky! hard code here
    fake_dynamic_points = all_dynamic_points - dynamic_mask 
    fake_dynamic_points[fake_dynamic_points < 0] = 0
    fake_dynamic_points = fake_dynamic_points.detach()

    dynamic_pred_dict = {}
    fake_dynamic_pred_dict = {}
    ...
    for viewpoint_cam in viewpoint_stack:
        ...
        dynamic_pred = feature_map[:1]
        dynamic_pred_dict[viewpoint_cam.image_name] = dynamic_pred.detach()
        save_heatmap(dynamic_pred.detach().squeeze(0), scene.model_path + "/dynamic_mask/" + f"{viewpoint_cam.image_name}_scores.png")
        dynamic_pred_mask = (dynamic_pred > cluster_threshold).float()
        save_heatmap(dynamic_pred_mask.detach().squeeze(0), scene.model_path + "/dynamic_mask/" + f"{viewpoint_cam.image_name}_mask.png")
        save_image(image.detach(), scene.model_path + "/dynamic_mask/" + f"{viewpoint_cam.image_name}_render.png")

        # compute fake dynamic mask
        render_pkg_fake = render_func(viewpoint_cam, gaussians, pipe, background, 
                                    camera_pose, deform, fake_dynamic_points, noise_scale,
                                    featkey, static)        
        fake_dynamic_pred = render_pkg_fake["feature_map"][:1]
        save_heatmap(fake_dynamic_pred.detach().squeeze(0), scene.model_path + "/dynamic_mask/" + f"{viewpoint_cam.image_name}_fake_scores.png")
        fake_dynamic_pred_mask = (fake_dynamic_pred > 0.15).float() # hacky! hard code here
        save_heatmap(fake_dynamic_pred_mask.detach().squeeze(0), scene.model_path + "/dynamic_mask/" + f"{viewpoint_cam.image_name}_fake_mask.png")
        fake_dynamic_pred_dict[viewpoint_cam.image_name] = fake_dynamic_pred.detach()

    import pdb; pdb.set_trace()

    # densify dynamic Gaussians
    selected_pts_mask = (dynamic_mask > 0.5).squeeze(1) # [N]
    gaussians.densify_dynamic_points(selected_pts_mask)
    added_dynamic_mask = torch.ones((int(dynamic_mask.sum()), 1), device='cuda')
    ...
    for iteration in range(opt.static_iterations + 1, opt.iterations + 1):
        ...
        # loss with handling fake dynamics
        if viewpoint_cam.uid == 0: # always fit into the first frame of image
            loss = (1.0 - opt.lambda_dssim) * l1_loss(image, gt_image) + opt.lambda_dssim * (1.0 - ssim(image, gt_image))
        else:
            fake_dynamic_mask = 1 - fake_dynamic_pred_dict[viewpoint_cam.image_name]
            loss = masked_l1_loss(image, gt_image, fake_dynamic_mask)
        
        """
        if viewpoint_cam.uid != -100: # always!!!
            loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image))
        else:
            loss = dynamic_l1_loss(image, gt_image, uncertainty_pred) + \
                    dynamic_uncertainty_loss(image, gt_image, uncertainty_pred, reg_coef=0.5)
        """

        # if iteration % 1000 == 0:
        #     with torch.no_grad():
        #         psnr_img = psnr(image, gt_image).mean()
        #         norm_map = torch.abs(gt_image - image).norm(dim=0, p=2).unsqueeze(0) # [1, H, W]
        #         norm_map = norm_map.detach() # as target to train dynamic model
        #         save_heatmap(norm_map.squeeze(0), scene.model_path + "/training_dynamics/" + f"{viewpoint_cam.image_name}_{iteration}_psnr_{psnr_img:.2f}.png")

        # dynamic constraint
        # dynamic_pred = feature_map[:1]
        # gt_dynamic_mask = dynamic_pred_dict[viewpoint_cam.image_name]
        # loss += 0.5 * l2_loss(dynamic_pred, gt_dynamic_mask)

...
def load_images(folder_or_list, size, square_ok=False, save_new_path=False):
    ...
    for path in folder_content:
        ...
        if save_new_path:
            new_root = root.replace('images', 'images_resized')
            os.makedirs(new_root, exist_ok=True)
            save_path = os.path.join(new_root, path)
            img.save(save_path)
    ...
    if save_new_path:
        # here we use the new size as the original size
        # hence no need to resize camera intrinsics, depth, pointmaps, etc.
        return imgs, (W2,H2)
    else:
        return imgs, (W1,H1)
...
def save_depth_map(ori_size, depth_maps, img_folder_path, train_img_list):
    depth_folder_path = img_folder_path.replace("images", "depth")
    os.makedirs(depth_folder_path, exist_ok=True)
    for (depth_map, train_img) in zip(depth_maps, train_img_list):
        # depth_map -> (H, W) ; resize size (W', H') from original size
        # depth map here is metric depth, unnormaized
        depth_map = cv2.resize(depth_map, ori_size, interpolation=cv2.INTER_LINEAR)
        # save original metric depth as numpy array
        np.save(os.path.join(depth_folder_path, train_img.replace('.png', '_depth.npy')), depth_map)
        # also save the depth map as image for visualization
        rgb_depth_map = cv2.normalize(depth_map, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
        rgb_depth_map = cv2.applyColorMap(rgb_depth_map, cv2.COLORMAP_JET)
        cv2.imwrite(os.path.join(depth_folder_path, train_img.replace('.png', '_depth.png')), rgb_depth_map)


def save_conf_map(ori_size, conf_maps, img_folder_path, train_img_list):
    conf_folder_path = img_folder_path.replace("images", "confidence")
    os.makedirs(conf_folder_path, exist_ok=True)
    for (conf_map, train_img) in zip(conf_maps, train_img_list):
        # original conf_map is stored in log format, we transform it into exp, ranging from 0 to +inf
        conf_map = np.exp(conf_map)
        # conf_map -> (H, W) ; resize size (W', H') from original size
        conf_map = cv2.resize(conf_map, ori_size, interpolation=cv2.INTER_LINEAR)
        # save original metric depth as numpy array
        np.save(os.path.join(conf_folder_path, train_img.replace('.png', '_conf.npy')), conf_map)
        # also save the confidence map as image for visualization
        rgb_conf_map = cv2.normalize(conf_map, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
        rgb_conf_map = cv2.applyColorMap(rgb_conf_map, cv2.COLORMAP_JET)
        cv2.imwrite(os.path.join(conf_folder_path, train_img.replace('.png', '_conf.png')), rgb_conf_map)

def save_point_map(ori_size, pts3d, img_folder_path, train_img_list):
    pts3d_folder_path = img_folder_path.replace("images", "pointmaps")
    os.makedirs(pts3d_folder_path, exist_ok=True)
    for (pts3d_img, train_img) in zip(pts3d, train_img_list):
        # pts3d_img -> (H, W, 3) ; resize size (W', H') from original size
        # pts3d_img here is in world coordinate, not in camera coordinate
        np.save(os.path.join(pts3d_folder_path, train_img.replace('.png', '_pts3d.npy')), pts3d_img)

...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def __init__(self, *args, optimize_pp=False, focal_break=20, **kwargs):
        ...
        self.im_depthmaps = nn.ParameterList([nn.Parameter(torch.randn(H, W)/10-3, requires_grad=True) for H, W in self.imshapes])  # log(depth)
        # self.im_poses = nn.ParameterList(self.rand_pose(self.POSE_DIM) for _ in range(self.n_imgs))  # camera poses
        self.im_poses = nn.ParameterList([nn.Parameter(self.rand_pose(self.POSE_DIM), requires_grad=True) for _ in range(self.n_imgs)])  # camera poses
        # self.im_focals = nn.ParameterList(torch.FloatTensor(
        #     [self.focal_break*np.log(max(H, W))]) for H, W in self.imshapes)  # camera intrinsics
        self.im_focals = nn.ParameterList([nn.Parameter(torch.FloatTensor(
            [self.focal_break*np.log(max(H, W))]), requires_grad=True) for H, W in self.imshapes])  # camera intrinsics

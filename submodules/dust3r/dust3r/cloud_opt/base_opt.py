...
class BasePCOptimizer (nn.Module):
    ...
    # Modified to avoid the following error:
    # raise TypeError("cannot assign '{}' object to parameter '{}' "
    # TypeError: cannot assign 'torch.FloatTensor' object to parameter
    # @torch.no_grad()
    ...
    def _compute_img_conf(self, pred1_conf, pred2_conf):
        ...
        for e, (i, j) in enumerate(self.edges):
            im_conf[i] = nn.Parameter(torch.maximum(im_conf[i], pred1_conf[e]))
            im_conf[j] = nn.Parameter(torch.maximum(im_conf[j], pred2_conf[e]))
        return im_conf

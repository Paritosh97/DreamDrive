...
def make_pairs(imgs, scene_graph='complete', prefilter=None, symmetrize=True):
    ...
    elif scene_graph == 'video':
        # each image is connected to its last image and the first frame (non-generated frame)
        for j in range(0, len(imgs)):
            if j == 0:
                pairs.append((imgs[0], imgs[0])) # itself for mono depth
            else:
                pairs.append((imgs[j], imgs[0])) # pointmap wrt. the first frame
                pairs.append((imgs[j], imgs[j-1])) # neighbor frames

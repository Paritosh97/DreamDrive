...
class BaseDiffusionSampler:
    ...
    def denoise(self, x, denoiser, sigma, cond, cond_mask, uc):
        denoised, feats = denoiser(*self.guider.prepare_inputs(x, sigma, cond, cond_mask, uc))
...
class EulerEDMSampler(SingleStepDiffusionSampler):
    ...
    def sampler_step(self, sigma, next_sigma, denoiser, x, cond, cond_mask=None, uc=None, gamma=0.0):
        ...
        denoised, feat = self.denoise(x, denoiser, sigma_hat, cond, cond_mask, uc)
        d = to_d(x, sigma_hat, denoised) # (x - denoised) / sigma_hat
        dt = append_dims(next_sigma - sigma_hat, x.ndim)

        euler_step = self.euler_step(x, d, dt) # x + dt * d
        return euler_step, feat
    ...
    def __call__(
        ...
        fl = [] # grasped feature list
        ts = [num_sigmas - 2] # timestep for grasping features, now choose last timestep

        for i in self.get_sigma_gen(num_sigmas): # multi-step diffusion iterations
            ...
            print("step", i, "sigma", sigmas[i], "next_sigma", sigmas[i + 1], "gamma", gamma)
            if i in ts:
                fl.append(f)
